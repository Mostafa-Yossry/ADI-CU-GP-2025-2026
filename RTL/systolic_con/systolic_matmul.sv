...
//
// Fixed-point formats (length=12 in MATLAB call):
...
//   After rounding: Q5.7,   12-bit  (WL_OUT = 12)  [RIGHT_SH=23, convergent]
//   Output Z      : Q5.7,   12-bit  — matches MATLAB T.Q1_11 (fi(1,12,7))
//
// Parameters (defaults for matched filter):
//   ROWS    = 8    (rows of H^H = elements of output Z)
//   COLS    = 1    (Z is 8x1)
//   K_DEPTH = 8    (dot-product length)
//   WL_IN   = 12   (Q1.11 input boundary)
//   WL_INT  = 16   (Q1.15 internal after widening)
//   WL_OUT  = 12   (Q5.7 output — matches MATLAB length=12)
//
...
module systolic_matmul #(
    ...
    parameter WL_IN   = 12,
    parameter WL_INT  = 16,
    parameter WL_OUT  = 12    // Q5.7   12-bit — matches MATLAB length=12
)(
    input  wire clk,
    input  wire rst_n,
    input  wire en,
