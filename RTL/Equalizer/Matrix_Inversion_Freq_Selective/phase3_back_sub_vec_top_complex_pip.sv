// ============================================================
// Module : phase3_back_sub_vec_top_complex_pip  (CONFIGURABLE NxN)
...
//               Before any stage executes, L and inv are pre-delayed by N cycles
//               (ALIGN_DELAY) via a d_ff / d_ff_complex chain to compensate for the
//               phase2 pipeline latency so the correct L elements arrive in lockstep
//               with the incoming z values. Within each stage, z[N-1-k] is delayed
...
module phase3_back_sub_vec_top_complex_pip #(
           parameter                N                       = 8             ,
           parameter                WIDTH                   = 16            ,
...
/*...........................................Local Parameters..................................................*/
    ...
    // ----------------------------------------------------------------
    // 0. Input Alignment (Delay L and inv_L by N-1 cycles) (Parameters)
    //    Compensates for Phase 2 latency so L aligns with incoming z
    // ----------------------------------------------------------------

    localparam ALIGN_DELAY = N ;

/*...........................................Interal Signals..................................................*/

    // ----------------------------------------------------------------
    // 0. Input Alignment (Delay L and inv_L by N-1 cycles) (Signals)
    //    Compensates for Phase 2 latency so L aligns with incoming z
    // ----------------------------------------------------------------
    
    wire signed [WIDTH-1:0] L_re_align [0:ALIGN_DELAY][0:N-1][0:N-1];
    wire signed [WIDTH-1:0] L_im_align [0:ALIGN_DELAY][0:N-1][0:N-1];
    wire signed [WIDTH-1:0] inv_align  [0:ALIGN_DELAY][0:N-1]       ;

    // ----------------------------------------------------------------
    // 1. L and inv pipeline (Depth N-1)                    (Signals)
    // ----------------------------------------------------------------
    
    wire signed [WIDTH-1:0] L_re_pipe  [0:N-1][0:N-1][0:N-1]        ;
    wire signed [WIDTH-1:0] L_im_pipe  [0:N-1][0:N-1][0:N-1]        ;
    wire signed [WIDTH-1:0] inv_pipe   [0:N-1][0:N-1]               ;

    // ----------------------------------------------------------------
    // 2. z pipeline (Depth N-1)                           (Signals)
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // 3. x pipeline (Feedback delays)                     (Signals)
    ...
    // ----------------------------------------------------------------
    // 6. Valid Out Pipeline (Latency = N-1 cycles)        (Signals)
    // ----------------------------------------------------------------
...
/*...........................................Internal Logic..................................................*/
    ...
    // ----------------------------------------------------------------
    // 0. Input Alignment (Delay L and inv_L by N-1 cycles)
    //    Compensates for Phase 2 latency so L aligns with incoming z
    // ----------------------------------------------------------------

    genvar Pipe_L_row_num, Pipe_L_col_num, Pipe_L_delay_stage_num;
    generate
        for (Pipe_L_row_num = 0; Pipe_L_row_num < N; Pipe_L_row_num = Pipe_L_row_num + 1) begin : align_in_row
            for (Pipe_L_col_num = 0; Pipe_L_col_num < N; Pipe_L_col_num = Pipe_L_col_num + 1) begin : align_in_col
                assign L_re_align[0][Pipe_L_row_num][Pipe_L_col_num] = L_re[Pipe_L_row_num][Pipe_L_col_num];
                assign L_im_align[0][Pipe_L_row_num][Pipe_L_col_num] = L_im[Pipe_L_row_num][Pipe_L_col_num];
            end
            assign inv_align[0][Pipe_L_row_num] = inv_L[Pipe_L_row_num];
        end
        
        for (Pipe_L_delay_stage_num = 0; Pipe_L_delay_stage_num < ALIGN_DELAY; Pipe_L_delay_stage_num = Pipe_L_delay_stage_num + 1) begin : align_stages
            for (Pipe_L_row_num = 0; Pipe_L_row_num < N; Pipe_L_row_num = Pipe_L_row_num + 1) begin : align_stage_row
                for (Pipe_L_col_num = 0; Pipe_L_col_num < N; Pipe_L_col_num = Pipe_L_col_num + 1) begin : align_stage_col
                    d_ff_complex #(
                            .WIDTH                (             WIDTH                                                                      ) 
                    ) eq_align_lp_ff (
                            .clk                  (             clk                                                                        ),
                            .rst                  (             rst                                                                        ),
                            .d_r                  (             L_re_align[Pipe_L_delay_stage_num  ][Pipe_L_row_num][Pipe_L_col_num]       ),
                            .d_im                 (             L_im_align[Pipe_L_delay_stage_num  ][Pipe_L_row_num][Pipe_L_col_num]       ),
                            .q_r                  (             L_re_align[Pipe_L_delay_stage_num+1][Pipe_L_row_num][Pipe_L_col_num]       ),
                            .q_im                 (             L_im_align[Pipe_L_delay_stage_num+1][Pipe_L_row_num][Pipe_L_col_num]       )
                    );
                end
                ...
                d_ff #(
                            ...
                            .clk                  (             clk                                                                        ),
                            .rst                  (             rst                                                                        ),
                            .d                    (             inv_align[Pipe_L_delay_stage_num  ][Pipe_L_row_num]                        ),
                            .q                    (             inv_align[Pipe_L_delay_stage_num+1][Pipe_L_row_num]                        )
                );
            end
        end
    endgenerate
    ...
    // ----------------------------------------------------------------
    // 1. L and inv pipeline (Depth N-1)
    // ----------------------------------------------------------------

    genvar L_row_num, L_col_num, L_delay_num;
    generate
        for (L_row_num = 0; L_row_num < N; L_row_num = L_row_num + 1) begin : L_d0_row
            for (L_col_num = 0; L_col_num < N; L_col_num = L_col_num + 1) begin : L_d0_col
                assign L_re_pipe[0][L_row_num][L_col_num] = L_re_align[ALIGN_DELAY][L_row_num][L_col_num]; 
                assign L_im_pipe[0][L_row_num][L_col_num] = L_im_align[ALIGN_DELAY][L_row_num][L_col_num]; 
            end
            assign inv_pipe[0][L_row_num] = inv_align[ALIGN_DELAY][L_row_num];             
        end
        
        for (L_delay_num = 0; L_delay_num < N-1; L_delay_num = L_delay_num + 1) begin : L_pipe_stages
            for (L_row_num = 0; L_row_num < N; L_row_num = L_row_num + 1) begin : L_pipe_row
                for (L_col_num = 0; L_col_num < N; L_col_num = L_col_num + 1) begin : L_pipe_col
                    d_ff_complex #(
                            .WIDTH                (             WIDTH                                                                      ) 
                    ) eq_lp_ff (
                            .clk                  (             clk                                                                        ),
                            .rst                  (             rst                                                                        ),
                            .d_r                  (             L_re_pipe[L_delay_num  ][L_row_num][L_col_num]                             ),
                            .d_im                 (             L_im_pipe[L_delay_num  ][L_row_num][L_col_num]                             ),
                            .q_r                  (             L_re_pipe[L_delay_num+1][L_row_num][L_col_num]                             ),
                            .q_im                 (             L_im_pipe[L_delay_num+1][L_row_num][L_col_num]                             )
                    );
                end
                ...
                d_ff #(
                            ...
                            .clk                  (             clk                                                                        ),
                            .rst                  (             rst                                                                        ),
                            .d                    (             inv_pipe[L_delay_num  ][L_row_num]                                         ),
                            .q                    (             inv_pipe[L_delay_num+1][L_row_num]                                         )
                );
            end
        end
    endgenerate
        ...
        for (inv_stage_num = 0; inv_stage_num < N; inv_stage_num = inv_stage_num + 1) begin : stage_k
            ...
            for (term_num = 0; term_num < inv_stage_num; term_num = term_num + 1) begin : gen_mults
                ...
                complex_mult_comb #(
                            ...
                            .b_re                 (              L_re_pipe[inv_stage_num][LROW][L_COL]                                     ), // Conjugate (+Re)
                            .b_im                 (             -L_im_pipe[inv_stage_num][LROW][L_COL]                                     ), // Conjugate (-Im)
                            .res_re               (             prod_re                                                                    ),
                            .res_im               (             prod_im                                                                    )
                );
            ...
            complex_mult_comb #(
                            ...
                            .a_re                 (             sub_re                                                                     ),
                            .a_im                 (             sub_im                                                                     ),
                            .b_re                 (             inv_pipe[inv_stage_num][L_COL]                                             ),
    ...
    // ----------------------------------------------------------------
    // 6. Valid Out Pipeline (Latency = N-1 cycles)
    // ----------------------------------------------------------------
