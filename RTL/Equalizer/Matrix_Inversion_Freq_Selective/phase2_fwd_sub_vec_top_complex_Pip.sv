// ============================================================
// Module : phase2_fwd_sub_vec_top_complex_Pip  (CONFIGURABLE NxN)
...
//               d_ff_complex chain; L and inv are pipelined one stage per cycle so
//               stage i sees L_pipe[i]; and z[j] (j<i) is tapped from its own
//               d_ff_complex delay chain at depth i-j so it arrives in the same
//               cycle as y[i]. The multiply-accumulate tree inside each stage uses
...
module phase2_fwd_sub_vec_top_complex_Pip #(
           parameter N                      = 8                     ,
           parameter WIDTH                  = 16                    ,
...
/*...........................................Internal Logic..................................................*/
    ...
    genvar row_number, col_number;
    generate
        for (row_number = 0; row_number < N; row_number = row_number + 1) begin : L_pipe_row_0
            for (col_number = 0; col_number < N; col_number = col_number + 1) begin : L_pipe_col_0
                assign L_re_pipe[0][row_number][col_number] = L_re[row_number][col_number];
                assign L_im_pipe[0][row_number][col_number] = L_im[row_number][col_number];
            end
            assign inv_pipe[0][row_number] = inv_L[row_number];
        end
    endgenerate

    genvar stage_number;
    generate
        for (stage_number = 0; stage_number < N-1; stage_number = stage_number + 1) begin : L_pipe_stages
            for (row_number = 0; row_number < N; row_number = row_number + 1) begin : L_pipe_stage_row
                for (col_number = 0; col_number < N; col_number = col_number + 1) begin : L_pipe_stage_col
                    d_ff_complex #(
                            .WIDTH                (             WIDTH                                                                      ) 
                    ) eq_lp_ff (
                            .clk                  (             clk                                                                        ),
                            .rst                  (             rst                                                                        ),
                            .d_r                  (             L_re_pipe[stage_number  ][row_number][col_number]                          ),
                            .d_im                 (             L_im_pipe[stage_number  ][row_number][col_number]                          ),
                            .q_r                  (             L_re_pipe[stage_number+1][row_number][col_number]                          ),
                            .q_im                 (             L_im_pipe[stage_number+1][row_number][col_number]                          )
                    );
                end
                d_ff #(
                            .WIDTH                (             WIDTH                                                                      ) 
                            ...
                            .clk                  (             clk                                                                        ),
                            .rst                  (             rst                                                                        ),
                            .d                    (             inv_pipe[stage_number  ][row_number]                                       ),
                            .q                    (             inv_pipe[stage_number+1][row_number]                                       )
                );
            end
        end
    endgenerate
        ...
        for (current_stage_number = 0; current_stage_number < N; current_stage_number = current_stage_number + 1) begin : stage_i
            ...
            for (current_term_number = 0; current_term_number < current_stage_number; current_term_number = current_term_number + 1) begin : gen_mults
                ...
                complex_mult_comb #(
                            ...
                            .a_re                 (             L_re_pipe[current_stage_number][current_stage_number][current_term_number] ),
                            .a_im                 (             L_im_pipe[current_stage_number][current_stage_number][current_term_number] ),
            ...
            complex_mult_comb #(
                            ...
                            .a_re                 (             sub_re                                                                     ),
                            .a_im                 (             sub_im                                                                     ),
                            .b_re                 (             inv_pipe[current_stage_number][current_stage_number]                       ),
