...
Computes `g_y = H^H · y` by chaining `hermitian_pipe` (computes H^H from H) into `matched_filter_pipe_wrap` (computes H^H · y). Black-box integration wrapper — internals are pre-verified IP.
...
| `x_valid` | out | 1 | g_y output valid |
| `x_re_flat` / `x_im_flat` | out | `N*MF_WL_OUT` | g_y = H^H·y, flattened, same packing convention |
...
3. Starting **2 cycles after `h_valid`**, stream `y` vectors on `y_re_flat`/`y_im_flat` with `y_valid` pulsed each cycle (back-to-back streaming supported).
4. `x_valid` pulses high when `g_y` is valid on `x_re_flat`/`x_im_flat`.
...
**13 cycles** from `h_valid` to the first `x_valid` (1 hermitian + 1 coef-hold settle + 1 hh_load register + 10 MF pipeline).

⚠️ The in-file header comment states 12 cycles, but the actual `hh_load_int` register stage (added to fix a coefficient-latching race, see RTL comments) makes the real total **13**. Verify against simulation before relying on this number.

Once `y_valid` begins streaming (2 cycles after `h_valid`), each `y` vector takes **10 cycles** end-to-end through the MF core, with back-to-back streaming supported (no bubble required between vectors).
