...
//   - Whether ANY output is valid on that cycle  (throughput slot used/idle)
//   - Frame latency = last_row_first_valid - start_cycle
// =============================================================================
...
// Free-running cycle counter, resets with rst_n
...
begin
    ...
    for (r=0; r<ROWS; r=r+1) begin hh_real[r]=0; hh_imag[r]=0; end
    y_real[0]=0; y_imag[0]=0;
    @(posedge clk); @(posedge clk);
    rst_n = 1;
    for (idle=0; idle<(ROWS+K_DEPTH); idle=idle+1) @(posedge clk);
end
endtask
...
begin
    for (k=0; k<K_DEPTH; k=k+1) begin
        ...
        y_real[0] = y_r_mem[k]; y_imag[0] = y_i_mem[k];
    end
    @(negedge clk);
    start = 0;
    for (r=0; r<ROWS; r=r+1) begin hh_real[r]=0; hh_imag[r]=0; end
...
//
// Prints one line per clock cycle covering:
//   Cycle | START | valid per row R0..R7 | ANY_VALID | Event annotation
//
// Then prints the frame summary table.
////////////////////////////////////////////////////////////////////////////////
...
begin
    ...
    while ((done_cnt < ROWS) && (watchdog < MAX_CYCLES)) begin
        ...
        // Capture start_cycle on the posedge that sees start=1
        ...
        for (row=0; row<ROWS; row=row+1) begin
            ...
            if (row_valid[row]) begin
                any_valid = 1;

                // Capture latest valid data
                ...
                // First valid for this row
                if (!first_valid_seen[row]) begin
        ...
        // Track last cycle with any output valid (frame end)
        if (any_valid) frame_end = cycle_counter;

        // ------------------------------------------------------------------
        // Print every cycle from start_cycle onwards, plus the start cycle
        // ------------------------------------------------------------------
        if ((start_cycle >= 0) && (cycle_counter >= start_cycle)) begin

            $write("  | %5d |   %0d   ",
                   cycle_counter,
                   (cycle_counter == start_cycle) ? 1 : 0);

            for (row=0; row<ROWS; row=row+1)
                $write("|  %0d ", row_valid[row]);

            $write("|  %0d  | ", any_valid);

            // Event annotation
            if (cycle_counter == start_cycle) begin
                $write("FRAME START (k=0 driven)      ");
            end else if (any_valid) begin
                // List rows becoming valid for first time this cycle
                for (row=0; row<ROWS; row=row+1) begin
                    if (row_valid[row] && (first_valid_cycle[row] == cycle_counter))
                        $write("row%0d VALID ", row);
                end
                // Pad to fixed width
                $write("                              ");
            end else begin
                $write("pipeline filling...           ");
            end
    ...
    $display("  +----------------------------------------------------------+");
    $display("  | Frame START cycle (start=1 posedge) : cycle %4d        |", start_cycle);
    $display("  | Frame END   cycle (last valid pulse): cycle %4d        |", frame_end);
    $display("  | Frame latency (end - start)         : %4d cycles       |",
             (frame_end >= 0 && start_cycle >= 0) ? (frame_end - start_cycle) : -1);
    $display("  +----------------------------------------------------------+");
    $display("  | Row | First Valid Cycle | Latency from START             |");
    $display("  +-----+------------------+--------------------------------+");

    for (row=0; row<ROWS; row=row+1) begin
        if (first_valid_cycle[row] >= 0) begin
            frame_lat = first_valid_cycle[row] - start_cycle;
            $display("  |  %0d  |     cycle %4d   |   %2d cycles                   |",
                     row, first_valid_cycle[row], frame_lat);
            // Accumulate global stats
            if (frame_lat < lat_min) lat_min = frame_lat;
            if (frame_lat > lat_max) lat_max = frame_lat;
            lat_sum   = lat_sum + frame_lat;
            lat_count = lat_count + 1;
        end else begin
            $display("  |  %0d  |     TIMEOUT     |   --                           |", row);
        end
    end
    ...
    if (watchdog >= MAX_CYCLES) begin
        $display("  !! TIMEOUT test=%0d done_cnt=%0d !!", test, done_cnt);
        for (row=0; row<ROWS; row=row+1)
            if (valid_count[row] < K_DEPTH) fail_cnt = fail_cnt + 1;
    end
end
endtask
...
begin
    for (row=0; row<ROWS; row=row+1) begin
        ...
        err_r = got_r[row] - exp_r; if (err_r < 0.0) err_r = -err_r;
        err_i = got_i[row] - exp_i; if (err_i < 0.0) err_i = -err_i;

        if ((err_r > TOL) || (err_i > TOL)) begin
            $display("  FAIL row=%0d  got=(%.6f,%.6f)  exp=(%.6f,%.6f)  err=(%.6f,%.6f)",
                row, got_r[row], got_i[row], exp_r, exp_i, err_r, err_i);
            fail_cnt = fail_cnt + 1;
        end else begin
            $display("  PASS row=%0d  got=(%.6f,%.6f)  exp=(%.6f,%.6f)",
                row, got_r[row], got_i[row], exp_r, exp_i);
            pass_cnt = pass_cnt + 1;
        end
    end
end
...
initial begin
    ...
    $display("  ROWS=%0d  COLS=%0d  K_DEPTH=%0d  WL_IN=%0d  WL_OUT=%0d",
             ROWS, COLS, K_DEPTH, WL_IN, WL_OUT);
    $display("  Expected: row0 first valid = START+%0d  row7 = START+%0d",
             K_DEPTH+1, K_DEPTH+ROWS);
    $display("==========================================================");
    ...
    $display("");
    $display("==========================================================");
    $display("  GLOBAL LATENCY SUMMARY  (%0d row measurements)", lat_count);
    ...
    $display("  Expected     : %0d cycles (row0) .. %0d cycles (row7)",
             K_DEPTH+1, K_DEPTH+ROWS);
    $display("==========================================================");
...
initial begin #10000000; $display("GLOBAL TIMEOUT"); $finish; end
